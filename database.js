...
async function getOverallStatistic(stat_type){
  ...
  const result = await db.get('SELECT name, user AS users, value FROM statistics INNER JOIN statistic_types ON statistic_types.id = type WHERE type=? GROUP BY type;', type_id);
  db.close();
